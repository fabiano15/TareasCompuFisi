...
logRango = [0]*20
logFrec = [0]*20
quizcolumnas=open("quiz_"+archivo,"w");

escribir1="Rango                      Frecuencia"
quizcolumnas.write(escribir1);
for ran in rango:
	rango[l-1]=l;
	frec[l-1]=matriz[0][l];


	logRango[l-1]=np.log(rango[l-1]);
	logFrec[l-1]=np.log(frec[l-1]/100);

	escribir2= str(rango[l-1]) + "                           "+str(frec[l-1])+"\n";
	quizcolumnas.write(escribir2);

	l=l+1;


polifit=np.polyfit(logRango,logFrec,1);

escribir="La constante encontrada en "+ archivo +" es:" +str(polifit[0])+"\n";
quizcolumnas.write(escribir);
