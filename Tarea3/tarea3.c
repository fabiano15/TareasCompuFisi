...
////////inicia codigo silva//////////
	...
	printf("la matriz\n");
	print_matrix(matriz2,filas2,columnas2);	

	transpuesta2=transpose(matriz2,columnas2,filas2);
	

	covarianza = cov_calculate2(matriz2);
	printf("la coovarianza\n");
	print_matrix(covarianza,matriz2->size2,matriz2->size2);	



	/*find_eigens2(matriz2, eigenvals, eigenvecs);     UTILIZAR EL METODO DEL PROFESOR
	printf("Eigen Vectors\n");
	print_matrix(eigenvecs,columnas2,columnas2);
	printf("Eigen Values\n");
	print_vector(eigenvals,columnas2);*/
	
	find_eigens(matriz2, eigenvals,eigenvecs);   //BORRAR ESTE METODO Y UTILIZAR EL DE ARRIBA//
	printf("Eigen Vectors\n");
	print_matrix(eigenvecs,columnas2,columnas2);
	printf("Eigen Values\n");
	print_vector(eigenvals,columnas2);
  	...
  	for(i=0;i<n_x;i++){
		for(j=0;j<n_y;j++){
			...
			gsl_matrix_set (matriz, i, 2, vector[3*i+2]);
		  	...
		  	printf("%f %f %f",vector[3*i],vector[3*i+1],vector[3*i+2]);
		}
	  }
  }
